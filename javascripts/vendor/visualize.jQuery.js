...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			yLabelInterval: 61.5 //distance between y labels
		},options);
					...
					if(o.parseDirection == 'x'){
						self.find('tr:gt(0)').filter(o.rowFilter).each(function(i){
							...
							$(this).find('td').filter(o.colFilter).each(function(){
								dataGroups[i].points.push( parseFloat($(this).text()).toFixed(2) );
							});
						});
					}
					else {
		...
		var createChart = {
			...
			line: function(area){
				...
				//write X labels
				var xInterval = 246 / (xLabels.length -1);
				...
				$.each(xLabels, function(i){ 
					...
					label
						.css('margin-left', leftOffset-15)
						.addClass('label');
				});

				//write Y labels
				var yScale = 246 / totalYRange;
				var liBottom = 246 / (yLabels.length-1);
				...
				$.each(dataGroups,function(h){
					...
					$.each(points, function(i, v){
						ctx.fillStyle = "#00b6f1";
						ctx.beginPath();
						ctx.arc(integer, -(this*yScale+20), 10, 0, Math.PI*2, true); 
						ctx.closePath();
						ctx.fill();
						ctx.beginPath();
						var tmp = v;
						...
						ctx.font = '14px bold "Ubuntu"';
						ctx.fillText(v, integer-10, -(this*yScale - 5));
						integer+=xInterval;
					});

					integer = 15;
					ctx.moveTo(15,-(points[0]*yScale + 20));
					$.each(points, function(){
						ctx.lineTo(integer,-(this*yScale + 20));
						integer+=xInterval;
					});
